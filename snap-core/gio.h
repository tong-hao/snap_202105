...
/// Loads a (directed, undirected or multi) graph from a text file InFNm with 1
/// edge per line (whitespace separated columns, integer node ids).
template <class PGraph>
PGraph LoadEdgeList(const TStr &InFNm, const int &SrcColId = 0,
                    const int &DstColId = 1);
/// Loads a (directed, undirected or multi) graph from a text file InFNm with 1
/// edge per line ('Separator' separated columns, integer node ids).
template <class PGraph>
PGraph LoadEdgeList(const TStr &InFNm, const int &SrcColId, const int &DstColId,
                    const char &Separator);
/// Loads a network from the text file InFNm with 1 node/edge per line
/// ('Separator' separated columns, integer node id(s) + node/edge attributes).
PNEANet LoadEdgeListNet(const TStr &InFNm, const char &Separator);
/// Loads a (directed, undirected or multi) graph from a text file InFNm with 1
/// edge per line (whitespace separated columns, arbitrary string node ids).
template <class PGraph>
...
PGraph LoadEdgeListStr(const TStr &InFNm, const int &SrcColId,
                       const int &DstColId, TStrHash<TInt> &StrToNIdH);
/// Loads a (directed, undirected or multi) graph from a text file InFNm with 1
/// node and all its edges in a single line.
...
/// Loads a directed network in the DyNetML format. Loads only the first network
/// in the file FNm.
PNGraph LoadDyNet(const TStr &FNm);
/// Loads directed networks in the DyNetML format. Loads all the networks in the
/// file FNm.
TVec<PNGraph> LoadDyNetGraphV(const TStr &FNm);

// TODO: Sparse/Dense adjacency matrix which values we threshold at Thresh to
// obtain an adjacency matrix. template <class PGraph> PGraph LoadAdjMtx(const
// TStr& FNm, const int Thresh);
// TODO: Load from a GML file format
// (http://en.wikipedia.org/wiki/Graph_Modelling_Language) template <class
// PGraph> PGraph LoadGml(const TStr& FNm, const int Thresh);

/// Saves a graph into a text file. Each line contains two columns and encodes a
/// single edge: <source node id><tab><destination node id>
template <class PGraph>
void SaveEdgeList(const PGraph &Graph, const TStr &OutFNm,
                  const TStr &Desc = TStr());
/// Saves a network into a text file. Each line encodes either an edge or a
/// node, along with its attributes.
void SaveEdgeListNet(const PNEANet &Graph, const TStr &OutFNm,
                     const TStr &Desc);
...
template <class PGraph>
void SavePajek(const PGraph &Graph, const TStr &OutFNm,
               const TIntStrH &NIdColorH, const TIntStrH &NIdLabelH);
/// Saves a graph in a Pajek .NET format.
template <class PGraph>
void SavePajek(const PGraph &Graph, const TStr &OutFNm,
               const TIntStrH &NIdColorH, const TIntStrH &NIdLabelH,
...
template <class PGraph>
void SaveGViz(const PGraph &Graph, const TStr &OutFNm,
              const TStr &Desc = TStr(), const bool &NodeLabels = false,
              const TIntStrH &NIdColorH = TIntStrH());
/// Save a graph in GraphVizp .DOT format.  ##SaveGViz_NIdLabelH
template <class PGraph>
void SaveGViz(const PGraph &Graph, const TStr &OutFNm, const TStr &Desc,
              const TIntStrH &NIdLabelH);

// TODO:  Save to a GML file format
// (http://en.wikipedia.org/wiki/Graph_Modelling_Language) template <class
// PGraph> SaveGml(const PGraph& Graph, const TStr& OutFNm, const TStr& Desc);
...
PGraph LoadEdgeList(const TStr &InFNm, const int &SrcColId,
                    const int &DstColId) {
  TSsParser Ss(InFNm, ssfCommaSep, true, true, true); // caton.hao
  PGraph Graph = PGraph::TObj::New();
  int SrcNId, DstNId;

  while (Ss.Next()) {
    if (!Ss.GetInt(SrcColId, SrcNId) || !Ss.GetInt(DstColId, DstNId)) {
      continue;
    }
...
PGraph LoadEdgeListStr(const TStr &InFNm, const int &SrcColId,
  ...
  TStrHash<TInt> StrToNIdH(
      Mega(1), true); // hash-table mapping strings to integer node ids
  while (Ss.Next()) {
...
/// Loads the format saved by TSnap::SaveEdgeList(), where node IDs are strings
/// and mapping of strings to node ids are stored ##LoadEdgeListStr_StrToNIdH
template <class PGraph>
PGraph LoadEdgeListStr(const TStr &InFNm, const int &SrcColId,
                       const int &DstColId, TStrHash<TInt> &StrToNIdH) {
  TSsParser Ss(InFNm, ssfWhiteSep);
...
/// Loads Whitespace separated file of several columns: <source node id>
/// <destination node id1> <destination node id2> ##LoadConnList
template <class PGraph> PGraph LoadConnList(const TStr &InFNm) {
...
/// Loads Whitespace separated file of several columns: <source node id>
/// <destination node id1> <destination node id2>, with a mapping of strings to
/// node IDs. ##LoadConnListStr
template <class PGraph>
PGraph LoadConnListStr(const TStr &InFNm, TStrHash<TInt> &StrToNIdH) {
  TSsParser Ss(InFNm, ssfWhiteSep, true, true, true);
  PGraph Graph = PGraph::TObj::New();
...
template <class PGraph> PGraph LoadPajek(const TStr &InFNm) {
  ...
  while (Ss.Next()) {
    ...
    if (strstr(Ss[0], "*arcslist") != NULL ||
        strstr(Ss[0], "*edgeslist") != NULL) {
      EdgeList = false;
      break;
    }
...
/// See http://vlado.fmf.uni-lj.si/pub/networks/pajek/doc/pajekman.pdf for a
...
void SavePajek(const PGraph &Graph, const TStr &OutFNm,
  ...
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI();
    ...
    fprintf(F, "%d  \"%d\" ic %s fos 10\n", i + 1, NI.GetId(),
            NIdColorH.IsKey(NI.GetId()) ? NIdColorH.GetDat(NI.GetId()).CStr()
                                        : "Red");
    NIdToIdH.AddDat(NI.GetId(), i + 1);
  }
  if (HasGraphFlag(typename PGraph::TObj, gfDirected)) {
...
void SavePajek(const PGraph &Graph, const TStr &OutFNm,
  ...
  for (typename PGraph::TObj::TEdgeI EI = Graph->BegEI(); EI < Graph->EndEI();
    ...
    const int DstNId = NIdToIdH.GetDat(EI.GetDstNId());
    fprintf(F, "%d %d 1 c %s\n", SrcNId, DstNId,
            EIdColorH.IsKey(EI.GetId()) ? EIdColorH.GetDat(EI.GetId()).CStr()
                                        : "Black");
  }
  fclose(F);
}

/// Each line contains a tuple of 3 values: <source node id><tab><destination
/// node id><tab>1
template <class PGraph>
void SaveMatlabSparseMtx(const PGraph &Graph, const TStr &OutFNm) {
  FILE *F = fopen(OutFNm.CStr(), "wt");
...
template <class PGraph>
void SaveGViz(const PGraph &Graph, const TStr &OutFNm, const TStr &Desc,
              const bool &NodeLabels, const TIntStrH &NIdColorH) {
  ...
  // node  [shape=box, width=0.3, height=0.3, label=\"\", style=filled,
  // fillcolor=red]
  ...
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI();
    ...
    if (NIdColorH.IsKey(NI.GetId())) {
      fprintf(F, "  %d [style=filled, fillcolor=\"%s\"];\n", NI.GetId(),
              NIdColorH.GetDat(NI.GetId()).CStr());
    } else {
      fprintf(F, "  %d ;\n", NI.GetId());
    }
  }
  ...
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI();
    ...
    if (NI.GetOutDeg() == 0 && NI.GetInDeg() == 0 &&
        !NIdColorH.IsKey(NI.GetId())) {
      fprintf(F, "%d;\n", NI.GetId());
    } else {
      for (int e = 0; e < NI.GetOutDeg(); e++) {
        if (!IsDir && NI.GetId() > NI.GetOutNId(e)) {
          continue;
        }
        fprintf(F, "  %d %s %d;\n", NI.GetId(), IsDir ? "->" : "--",
                NI.GetOutNId(e));
