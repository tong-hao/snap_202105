...
/// Returns the BFS tree size (number of nodes) and depth (number of levels) by
/// following in-links (parameter FollowIn = true) and/or out-links (parameter
/// FollowOut = true) of node StartNId.
template <class PGraph>
int GetSubTreeSz(const PGraph &Graph, const int &StartNId,
                 const bool &FollowOut, const bool &FollowIn, int &TreeSzX,
                 int &TreeDepthX);
/// Finds IDs of all nodes that are at distance Hop from node StartNId.
/// ##GetSubTreeSz
template <class PGraph>
int GetNodesAtHop(const PGraph &Graph, const int &StartNId, const int &Hop,
                  TIntV &NIdV, const bool &IsDir = false);
/// Returns the number of nodes at each hop distance from the starting node
/// StartNId. ##GetNodesAtHops
template <class PGraph>
int GetNodesAtHops(const PGraph &Graph, const int &StartNId, TIntPrV &HopCntV,
...
/// ##GetShortPath1
template <class PGraph>
int GetShortPath(const PGraph &Graph, const int &SrcNId, const int &DstNId,
                 const bool &IsDir = false);
/// Returns the length of the shortest path from node SrcNId to all other nodes
/// in the network. ##GetShortPath2
template <class PGraph>
int GetShortPath(const PGraph &Graph, const int &SrcNId, TIntH &NIdToDistH,
                 const bool &IsDir = false, const int &MaxDist = TInt::Mx);

/////////////////////////////////////////////////
// Diameter

/// Returns the (approximation of the) Diameter (maximum shortest path length)
/// of a graph (by performing BFS from NTestNodes random starting nodes).
/// ##GetBfsFullDiam
template <class PGraph>
int GetBfsFullDiam(const PGraph &Graph, const int &NTestNodes,
                   const bool &IsDir = false);
/// Returns the (approximation of the) Effective Diameter (90-th percentile of
/// the distribution of shortest path lengths) of a graph (by performing BFS
/// from NTestNodes random starting nodes). ##GetBfsEffDiam1
template <class PGraph>
double GetBfsEffDiam(const PGraph &Graph, const int &NTestNodes,
                     const bool &IsDir = false);
/// Returns the (approximation of the) Effective Diameter and the Diameter of a
/// graph (by performing BFS from NTestNodes random starting nodes).
/// ##GetBfsEffDiam2
template <class PGraph>
double GetBfsEffDiam(const PGraph &Graph, const int &NTestNodes,
                     const bool &IsDir, double &EffDiamX, int &FullDiamX);
/// Returns the (approximation of the) Effective Diameter, the Diameter and the
/// Average Shortest Path length in a graph (by performing BFS from NTestNodes
/// random starting nodes). ##GetBfsEffDiam3
template <class PGraph>
double GetBfsEffDiam(const PGraph &Graph, const int &NTestNodes,
                     const bool &IsDir, double &EffDiamX, int &FullDiamX,
                     double &AvgSPLX);
/// Returns the (approximation of the) Effective Diameter, the Diameter and the
/// Average Shortest Path length in a graph (by performing BFS from NTestNodes
/// random starting nodes). ##GetBfsEffDiamAll
template <class PGraph>
double GetBfsEffDiamAll(const PGraph &Graph, const int &NTestNodes,
                        const bool &IsDir, double &EffDiamX, int &FullDiamX,
                        double &AvgSPLX);
/// Use the whole graph (all edges) to measure the shortest path lengths but
/// only report the path lengths between nodes in the SubGraphNIdV.
/// ##GetBfsEffDiam4
template <class PGraph>
double GetBfsEffDiam(const PGraph &Graph, const int &NTestNodes,
                     const TIntV &SubGraphNIdV, const bool &IsDir,
                     double &EffDiamX, int &FullDiamX);

// TODO: Implement in the future
// template <class PGraph> int GetRangeDist(const PGraph& Graph, const int&
// SrcNId, const int& DstNId, const bool& IsDir=false); template <class PGraph>
// int GetShortPath(const PGraph& Graph, const int& SrcNId, TIntH& NIdToDistH,
// const bool& IsDir=false, const int& MaxDist=1000); template <class PGraph>
// int GetShortPath(const PGraph& Graph, const int& SrcNId, const TIntSet&
// TargetSet, const bool& IsDir, TIntV& PathNIdV); template <class PGraph> int
// GetShortPath(TIntH& NIdPrnH, TCcQueue<int>& NIdQ, const PGraph& Graph, const
// int& SrcNId, const TIntSet& TargetSet, const bool& IsDir, TIntV& PathNIdV);
// template <class PGraph> int GetMxShortDist(const PGraph& Graph, const int&
// SrcNId, const bool& IsDir=false); template <class PGraph> int
// GetMxShortDist(const PGraph& Graph, const int& SrcNId, const bool& IsDir,
// int& MxDistNId); template <class PGraph> int GetMxShortDist(const PGraph&
// Graph, const int& SrcNId, const bool& IsDir, int& MxDistNId, TCcQueue<int>&
// NIdQ, TCcQueue<int>& DistQ, TIntSet& VisitedH); template <class PGraph> int
// GetMxGreedyDist(const PGraph& Graph, const int& SrcNId, const bool&
// IsDir=false); template <class PGraph> int GetMxGreedyDist(const PGraph&
// Graph, const int& SrcNId, const bool& IsDir, TCcQueue<int>& NIdQ,
// TCcQueue<int>& DistQ, TIntSet& VisitedH); template <class PGraph> PNGraph
// GetShortPathsSubGraph(const PGraph& Graph, const TIntV& SubGraphNIdV);
// template <class PGraph> PGraph GetWccPathsSubGraph(const PGraph& Graph, const
// TIntV& NIdV); template <class PGraph> void GetSubTreeSz(const PGraph& Graph,
// const int& StartNId, const bool& FollowOutEdges, int& TreeSz, int&
// TreeDepth);
...
/// The class is meant for executing many BFSs over a fixed graph. This means
/// that the class can keep the hash tables and queues initialized between
/// different calls of the DoBfs() function.
template <class PGraph> class TBreathFS {
public:
  PGraph Graph;
...
public:
  ...
  /// Sets the graph to be used by the BFS to GraphPt and resets the data
  /// structures.
  void SetGraph(const PGraph &GraphPt);
  /// Performs BFS from node id StartNode for at maps MxDist steps by only
  /// following in-links (parameter FollowIn = true) and/or out-links (parameter
  /// FollowOut = true).
  int DoBfs(const int &StartNode, const bool &FollowOut, const bool &FollowIn,
            const int &TargetNId = -1, const int &MxDist = TInt::Mx);
  /// Same functionality as DoBfs with better performance.
  int DoBfsHybrid(const int &StartNode, const bool &FollowOut,
                  const bool &FollowIn, const int &TargetNId = -1,
  ...
  /// Note you have to first call DoBFs(). SrcNId must be equal to StartNode,
...
private:
  ...
  /* Private functions */
  bool TopDownStep(TIntV &NIdDistV, TIntV *Frontier, TIntV *NextFrontier,
                   int &MaxDist, const int &TargetNId, const bool &FollowOut,
                   const bool &FollowIn);
  bool BottomUpStep(TIntV &NIdDistV, TIntV *Frontier, TIntV *NextFrontier,
                    int &MaxDist, const int &TargetNId, const bool &FollowOut,
                    const bool &FollowIn);
};
...
void TBreathFS<PGraph>::SetGraph(const PGraph &GraphPt) {
  ...
  if (NIdDistH.GetReservedKeyIds() < N) {
    NIdDistH.Gen(N);
  }
}

template <class PGraph>
int TBreathFS<PGraph>::DoBfs(const int &StartNode, const bool &FollowOut,
                             const bool &FollowIn, const int &TargetNId,
                             const int &MxDist) {
  StartNId = StartNode;
  IAssert(Graph->IsNode(StartNId));
  //  const typename PGraph::TObj::TNodeI StartNodeI = Graph->GetNI(StartNode);
  //  IAssertR(StartNodeI.GetOutDeg() > 0, TStr::Fmt("No neighbors from start
  //  node %d.", StartNode));
  NIdDistH.Clr(false);
...
template <class PGraph>
int TBreathFS<PGraph>::DoBfsHybrid(const int &StartNode, const bool &FollowOut,
                                   const bool &FollowIn, const int &TargetNId,
                                   const int &MxDist) {
  StartNId = StartNode;
  IAssert(Graph->IsNode(StartNId));
  if (TargetNId == StartNode)
    return 0;
  const typename PGraph::TObj::TNodeI StartNodeI = Graph->GetNI(StartNode);
  ...
  while (!Frontier->Empty()) {
    ...
    if (Stage == 0 || Stage == 2) {
      targetFound = TopDownStep(NIdDistV, Frontier, NextFrontier, MaxDist,
                                TargetNId, FollowOut, FollowIn);
    } else {
      targetFound = BottomUpStep(NIdDistV, Frontier, NextFrontier, MaxDist,
                                 TargetNId, FollowOut, FollowIn);
    }
    if (targetFound) {
...
template <class PGraph>
bool TBreathFS<PGraph>::TopDownStep(TIntV &NIdDistV, TIntV *Frontier,
                                    TIntV *NextFrontier, int &MaxDist,
                                    const int &TargetNId, const bool &FollowOut,
                                    const bool &FollowIn) {
  for (TIntV::TIter it = Frontier->BegI(); it != Frontier->EndI();
       ++it) { // loop over frontier
    const int NId = *it;
...
template <class PGraph>
bool TBreathFS<PGraph>::BottomUpStep(TIntV &NIdDistV, TIntV *Frontier,
                                     TIntV *NextFrontier, int &MaxDist,
                                     const int &TargetNId,
                                     const bool &FollowOut,
                                     const bool &FollowIn) {
  for (typename PGraph::TObj::TNodeI NodeI = Graph->BegNI();
       NodeI < Graph->EndNI(); NodeI++) {
...
int TBreathFS<PGraph>::GetHops(const int &SrcNId, const int &DstNId) const {
  ...
  if (!NIdDistH.IsKeyGetDat(DstNId, Dist)) {
    return -1;
  }
  return Dist.Val;
}
...
template <class PGraph>
int TBreathFS<PGraph>::GetRndPath(const int &SrcNId, const int &DstNId,
                                  TIntV &PathNIdV) const {
  PathNIdV.Clr(false);
  if (SrcNId != StartNId || !NIdDistH.IsKey(DstNId)) {
    return -1;
  }
  PathNIdV.Add(DstNId);
  TIntV CloserNIdV;
...
template <class PGraph>
int GetSubTreeSz(const PGraph &Graph, const int &StartNId,
                 const bool &FollowOut, const bool &FollowIn, int &TreeSz,
                 int &TreeDepth) {
  TBreathFS<PGraph> BFS(Graph);
  BFS.DoBfs(StartNId, FollowOut, FollowIn, -1, TInt::Mx);
...
template <class PGraph>
int GetShortPath(const PGraph &Graph, const int &SrcNId, TIntH &NIdToDistH,
                 const bool &IsDir, const int &MaxDist) {
  TBreathFS<PGraph> BFS(Graph);
...
template <class PGraph>
double GetBfsEffDiam(const PGraph &Graph, const int &NTestNodes,
                     const bool &IsDir, double &EffDiam, int &FullDiam) {
  double AvgDiam;
  EffDiam = -1;
  FullDiam = -1;
...
double GetBfsEffDiam(const PGraph &Graph, const int &NTestNodes,
  ...
  for (int tries = 0; tries < TMath::Mn(NTestNodes, Graph->GetNodes());
       tries++) {
  ...
  EffDiam = TSnap::TSnapDetail::CalcEffDiamPdf(
      DistNbrsPdfV, 0.9);             // effective diameter (90-th percentile)
  FullDiam = DistNbrsPdfV.Last().Key; // approximate full diameter (max shortest
                                      // path length over the sampled nodes)
...
double GetBfsEffDiam(const PGraph &Graph, const int &NTestNodes,
  ...
  NodeIdV.Shuffle(TInt::Rnd);
  TInt Dist;
  for (int tries = 0; tries < TMath::Mn(NTestNodes, SubGraphNIdV.Len());
       tries++) {
    const int NId = NodeIdV[tries];
    BFS.DoBfs(NId, true, !IsDir, -1, TInt::Mx);
...
template <class PGraph>
int GetShortestDistances(const PGraph &Graph, const int &StartNId,
                         const bool &FollowOut, const bool &FollowIn,
                         TIntV &ShortestDists) {
  ...
  for (int NId = 0; NId < MxNId; NId++) {
    if (Graph->IsNode(NId)) {
      ShortestDists[NId] = InfDepth;
    } else {
      ShortestDists[NId] = NonNodeDepth;
    }
  }
...
#ifdef USE_OPENMP
template <class PGraph>
int GetShortestDistancesMP2(const PGraph &Graph, const int &StartNId,
                            const bool &FollowOut, const bool &FollowIn,
                            TIntV &ShortestDists) {
  int MxNId = Graph->GetMxNId();
  int NonNodeDepth = 2147483647; // INT_MAX
...
#pragma omp parallel for schedule(dynamic, 10000)
    for (int i = 0; i < PCurV->Len(); i++) {
      ...
      for (int e = 0; e < NI.GetOutDeg(); e++) {
        const int OutNId = NI.GetOutNId(e);
        if (__sync_bool_compare_and_swap(&(ShortestDists[OutNId].Val), InfDepth,
                                         Depth)) {
          PNextV->AddMP(OutNId);
        }
      }
